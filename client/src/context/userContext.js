...
export const UserProvider = (props) => {
    ...
    const sessionCheck = async (sid) => {
        ...
        .then( result => { setUser(result.data); navigate(location.pathname === '/Login' || location.pathname === '/Signin' || location.pathname.includes("/Home/Oauth") ? '/Home':location.pathname); console.log(result.data)}) // Change navigate route as needed
        .catch( error => handleError(error.response))
    }
